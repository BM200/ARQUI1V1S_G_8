...
// =====================================
// CALCULAR SUMA TOTAL
// Formula: MEDIA = SUM(X) / N
// =====================================
        ldr x21, =values             // puntero al arreglo
        mov x22, #0                  // contador i
        mov x23, #0                  // suma acumulada

ciclo_suma_varianza:
        cmp x22, x20
        b.ge fin_suma_varianza
        ldr x24, [x21], #8           // leer X_i
        add x23, x23, x24            // suma += X_i
        add x22, x22, #1             // i++
        b ciclo_suma_varianza

fin_suma_varianza:
        udiv x24, x23, x20           // x24 = media entera
        ldr x9, =mean_mem
        str x24, [x9]

// =====================================
// CALCULAR VARIANZA
// Formula: VAR = SUM((X - MEDIA)^2) / N
// =====================================
        ldr x21, =values
        mov x22, #0                  // contador i
...
ciclo_cuadrados:
        cmp x22, x20
        b.ge fin_cuadrados
        ldr x25, [x21], #8           // leer X_i
        sub x26, x25, x24            // diferencia = X_i - media
        mul x26, x26, x26            // diferencia^2
        add x23, x23, x26            // acumular cuadrados
        add x22, x22, #1
        b ciclo_cuadrados

fin_cuadrados:
        udiv x25, x23, x20           // x25 = varianza
        ldr x9, =variance_mem
        str x25, [x9]

// =====================================
// CALCULAR DESVIACION ESTANDAR
// Formula: STD_DEV = sqrt(VAR)
// =====================================
        mov x0, x25
        bl int_sqrt_u64
        ldr x9, =std_mem
...
// Entrada: x0 = SP original del proceso
