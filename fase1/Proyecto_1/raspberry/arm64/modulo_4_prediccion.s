...
// =====================================
// TOMAR VALOR INICIAL Y VALOR FINAL
// =====================================
        ldr x21, =values
        ldr x22, [x21]               // x22 = valor inicial X_1
        add x21, x21, #(29 * 8)      // avanzar hasta X_30
        ldr x23, [x21]               // x23 = valor final X_30
...
// Formula: DIF = XFINAL - XINICIAL
// =====================================
        sub x24, x23, x22            // x24 = diferencia total con signo
        ldr x9, =diff_mem
        str x24, [x9]

// =====================================
// CALCULAR PROMEDIO DE CAMBIO CON 2 DECIMALES
// Formula: PROMEDIO_CAMBIO = DIF / (N - 1)
// Para imprimir decimales, se usa escala x100:
// AVG_CHANGE_ESCALADO = DIF * 100 / 29
// =====================================
        mov x25, #100
        mul x26, x24, x25            // x26 = DIF * 100
        mov x25, #29                 // N - 1 = 29
        sdiv x26, x26, x25           // x26 = promedio de cambio escalado
        ldr x9, =avg_mem
        str x26, [x9]

// =====================================
// CALCULAR PREDICCION SIGUIENTE
// Formula: PREDICCION = XFINAL + PROMEDIO_CAMBIO
// Tambien se guarda escalada por 100 para imprimir 2 decimales.
// =====================================
        mov x25, #100
        mul x27, x23, x25            // x27 = XFINAL * 100
        add x27, x27, x26            // x27 = NEXT_VALUE escalado
        ldr x9, =next_mem
        str x27, [x9]
