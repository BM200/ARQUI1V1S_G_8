// ============================================================
// MODULO 3: DETECCION ESTADISTICA DE ANOMALIAS
...
msg_module:
        .asciz "MODULE=ANOMALY_DETECTION\n"
msg_total:
        .asciz "TOTAL_VALUES="
msg_mean:
        .asciz "MEAN="
...
// =====================================
// PRIMER CICLO: CALCULAR SUMA Y MEDIA
// =====================================
        ldr x21, =values
        mov x22, #0                  // contador
        mov x23, #0                  // suma

ciclo_suma_anomalias:
        cmp x22, x20
        b.ge fin_suma_anomalias
        ldr x24, [x21], #8
        add x23, x23, x24
        add x22, x22, #1
        b ciclo_suma_anomalias

fin_suma_anomalias:
        udiv x24, x23, x20           // x24 = media
        ldr x9, =mean_mem
        str x24, [x9]

// =====================================
// SEGUNDO CICLO: CALCULAR VARIANZA
// =====================================
        ldr x21, =values
        mov x22, #0
        mov x23, #0                  // suma de cuadrados

ciclo_varianza_anomalias:
        cmp x22, x20
        b.ge fin_varianza_anomalias
        ldr x25, [x21], #8
        sub x26, x25, x24            // diferencia = X_i - media
        mul x26, x26, x26            // diferencia^2
        add x23, x23, x26
        add x22, x22, #1
        b ciclo_varianza_anomalias

fin_varianza_anomalias:
        udiv x25, x23, x20           // varianza

// =====================================
// TERCER PASO: CALCULAR DESVIACION ESTANDAR
// =====================================
        mov x0, x25
        bl int_sqrt_u64
        mov x25, x0                  // x25 = STD_DEV
        ldr x9, =std_mem
        str x25, [x9]

// =====================================
// CUARTO CICLO: DETECTAR ANOMALIAS
// Regla equivalente a |Z| >= 2:
// |X - MEDIA| >= 2 * STD_DEV
// =====================================
        ldr x21, =values
        mov x22, #0                  // contador
        mov x23, #0                  // contador de anomalias
        add x26, x25, x25            // x26 = 2 * STD_DEV

        cmp x26, #0                  // si STD_DEV=0, no hay variacion suficiente
        beq fin_anomalias

ciclo_anomalias:
        cmp x22, x20
        b.ge fin_anomalias
        ldr x27, [x21], #8           // dato actual
        ldr x9, =mean_mem
        ldr x24, [x9]                // media guardada
        sub x27, x27, x24            // diferencia = dato - media
        cmp x27, #0
        b.ge diferencia_positiva
        neg x27, x27                 // valor absoluto si era negativo

diferencia_positiva:
        cmp x27, x26
        blt no_es_anomalia
        add x23, x23, #1             // anomalias++
...
// CLASIFICAR RIESGO
// 0 anomalias = NORMAL
// 1 a 3 anomalias = MEDIUM
// 4 o mas anomalias = HIGH
// =====================================
        ldr x9, =anomalies_mem
        ldr x23, [x9]
        cmp x23, #0
