...
msg_increments:
        .ascii "INCREMENTS="
        .equ MSG_INCREMENTS_LEN, . - msg_increments
msg_decrements:
        .ascii "DECREMENTS="
        .equ MSG_DECREMENTS_LEN, . - msg_decrements
msg_max_up:
        .ascii "MAX_UP_STREAK="
        .equ MSG_MAX_UP_LEN, . - msg_max_up
msg_max_down:
        .ascii "MAX_DOWN_STREAK="
        .equ MSG_MAX_DOWN_LEN, . - msg_max_down
msg_accum:
        .ascii "ACCUM_DIFF="
        .equ MSG_ACCUM_LEN, . - msg_accum
msg_trend:
        .ascii "TREND="
        .equ MSG_TREND_LEN, . - msg_trend
msg_trend_up:
        .ascii "UP"
        .equ MSG_TREND_UP_LEN, . - msg_trend_up
msg_trend_down:
        .ascii "DOWN"
        .equ MSG_TREND_DOWN_LEN, . - msg_trend_down
msg_trend_stable:
        .ascii "STABLE"
        .equ MSG_TREND_STABLE_LEN, . - msg_trend_stable
msg_error:
        .ascii "ERROR="
...
det_argumentos:
        .ascii "uso_modulo_5_tendencia_archivo_inicio_fin_columna"
...
conteo_guardado:
        .skip 8
incrementos_guardados:
        .skip 8
decrementos_guardados:
        .skip 8
racha_subida_guardada:
        .skip 8
racha_bajada_guardada:
        .skip 8
...
_start:
        ...
        // esto inicializa el recorrido de tendencia
        ldr x19, =valores
        ldr x20, [x19], #8
        mov x21, x1
        mov x22, #1
...
recorrer_tendencia:
        ...
        // esto calcula la diferencia contra el valor anterior
        ldr x10, [x19], #8
        sub x11, x10, x20
        add x9, x9, x11
        ...
        // esto reinicia rachas si el valor no cambia
        mov x25, #0
        mov x26, #0
        b avanzar_recorrido
...
avanzar_recorrido:
        mov x20, x10
        add x22, x22, #1
        b recorrer_tendencia

guardar_resultados:
        // esto guarda los resultados calculados
        ldr x10, =incrementos_guardados
        str x23, [x10]
        ldr x10, =decrementos_guardados
        str x24, [x10]
        ldr x10, =racha_subida_guardada
        str x27, [x10]
        ldr x10, =racha_bajada_guardada
        str x28, [x10]
        ldr x10, =diferencia_acumulada
        str x9, [x10]

        // esto selecciona la tendencia final
        cmp x9, #0
        bgt tendencia_subida
        blt tendencia_bajada
        ldr x0, =msg_trend_stable
        mov x1, #MSG_TREND_STABLE_LEN
        b guardar_tendencia

tendencia_subida:
        ldr x0, =msg_trend_up
        mov x1, #MSG_TREND_UP_LEN
        b guardar_tendencia

tendencia_bajada:
        ldr x0, =msg_trend_down
        mov x1, #MSG_TREND_DOWN_LEN
...
imprimir_ok:
        ...
        // esto imprime incrementos
        ldr x0, =msg_increments
        mov x1, #MSG_INCREMENTS_LEN
        ldr x2, =incrementos_guardados
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime decrementos
        ldr x0, =msg_decrements
        mov x1, #MSG_DECREMENTS_LEN
        ldr x2, =decrementos_guardados
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime la racha maxima de subida
        ldr x0, =msg_max_up
        mov x1, #MSG_MAX_UP_LEN
        ldr x2, =racha_subida_guardada
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime la racha maxima de bajada
        ldr x0, =msg_max_down
        mov x1, #MSG_MAX_DOWN_LEN
        ldr x2, =racha_bajada_guardada
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime la diferencia acumulada
        ldr x0, =msg_accum
        mov x1, #MSG_ACCUM_LEN
        ldr x2, =diferencia_acumulada
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime la tendencia final
        ldr x0, =msg_trend
