...
msg_error_integral:
        ...
        .equ MSG_ERROR_INTEGRAL_LEN, . - msg_error_integral
msg_error:
        .ascii "ERROR="
...
det_argumentos:
        .ascii "uso_modulo_4_integral_error_archivo_inicio_fin_columna_ideal"
...
conteo_guardado:
        .skip 8
error_previo_guardado:
        .skip 8
error_actual_guardado:
        .skip 8
area_guardada:
        .skip 8
...
_start:
        ...
        // esto calcula la integral del error
        ldr x19, =valores
        mov x20, x1
        mov x21, #0
        mov x22, #0
...
calcular_integral:
        ...
        // esto calcula el error absoluto actual
        ldr x24, [x19, x21, lsl #3]
        sub x0, x24, x23
        bl valor_absoluto
        mov x25, x0
        ldr x0, =error_actual_guardado
        str x25, [x0]

        cbz x21, guardar_primer_error

        // esto acumula el area del trapecio
        ldr x26, =error_previo_guardado
        ldr x26, [x26]
        add x27, x26, x25
        mov x28, #2
        udiv x27, x27, x28
...
guardar_primer_error:
        ldr x0, =error_previo_guardado
        str x25, [x0]
        add x21, x21, #1
        b calcular_integral

guardar_area:
        ldr x0, =area_guardada
        str x22, [x0]
...
imprimir_ok:
        ...
        // esto imprime la integral del error
        ldr x0, =msg_error_integral
        mov x1, #MSG_ERROR_INTEGRAL_LEN
        ldr x2, =area_guardada
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto
