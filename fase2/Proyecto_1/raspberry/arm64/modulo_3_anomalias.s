...
risk_normal:
        .ascii "NORMAL"
        .equ RISK_NORMAL_LEN, . - risk_normal
risk_medium:
        .ascii "MEDIUM"
        .equ RISK_MEDIUM_LEN, . - risk_medium
risk_high:
        .ascii "HIGH"
        .equ RISK_HIGH_LEN, . - risk_high
...
det_argumentos:
        .ascii "uso_modulo_3_anomalias_archivo_inicio_fin_columna"
...
calcular_media:
        ...
        // esto calcula la suma de cuadrados
        ldr x19, =valores
        mov x21, #0
        mov x22, #0
sumar_cuadrados:
        cmp x21, x20
        bge calcular_desviacion
        ldr x23, [x19, x21, lsl #3]
        sub x25, x23, x24
        mul x25, x25, x25
        add x22, x22, x25
...
calcular_desviacion:
        ...
        mov x0, x26
        bl raiz_entera
        ldr x1, =desviacion_guardada
        str x0, [x1]

        // esto cuenta anomalias con el criterio actual
        ldr x19, =valores
        mov x21, #0
        mov x22, #0
        ldr x23, =desviacion_guardada
        ldr x23, [x23]
        add x24, x23, x23
        cbz x24, guardar_anomalias
contar_anomalias:
        cmp x21, x20
        bge guardar_anomalias
        ldr x25, [x19, x21, lsl #3]
        ldr x26, =media_guardada
        ldr x26, [x26]
        sub x25, x25, x26
        cmp x25, #0
        bge diferencia_lista
        neg x25, x25
diferencia_lista:
        cmp x25, x24
        blt siguiente_anomalia
        add x22, x22, #1
siguiente_anomalia:
        add x21, x21, #1
        b contar_anomalias
...
guardar_anomalias:
        ldr x0, =anomalias_guardadas
        str x22, [x0]
        bl clasificar_riesgo
        bl imprimir_ok
        mov x0, #0
        mov x8, #93
        svc #0
...
clasificar_riesgo:
        // esto clasifica el riesgo por cantidad de anomalias
        ldr x0, =anomalias_guardadas
        ldr x0, [x0]
        cmp x0, #0
        beq riesgo_es_normal
        cmp x0, #4
        bge riesgo_es_alto
        ldr x1, =risk_medium
...
imprimir_ok:
        ...
        // esto imprime la cantidad de anomalias
        ldr x0, =msg_anomalies
        mov x1, #MSG_ANOMALIES_LEN
        ldr x2, =anomalias_guardadas
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime el riesgo del sistema
        ldr x0, =msg_risk
        mov x1, #MSG_RISK_LEN
        bl escribir_salida
        ldr x0, =riesgo_ptr
