.equ MAX_VALORES, 8192
        ...
        .equ COD_DENOMINADOR_INVALIDO, -12
...
msg_slope:
        .ascii "SLOPE_X100="
        .equ MSG_SLOPE_LEN, . - msg_slope
msg_trend:
        .ascii "TREND="
        .equ MSG_TREND_LEN, . - msg_trend
msg_error:
        .ascii "ERROR="
...
trend_ascending:
        .ascii "ASCENDING"
        .equ TREND_ASCENDING_LEN, . - trend_ascending
trend_descending:
        .ascii "DESCENDING"
        .equ TREND_DESCENDING_LEN, . - trend_descending
trend_stable:
        .ascii "STABLE"
        .equ TREND_STABLE_LEN, . - trend_stable
...
err_denominador:
        .ascii "invalid_denominator"
        .equ ERR_DENOMINADOR_LEN, . - err_denominador
err_desconocido:
        .ascii "unknown_error"
        .equ ERR_DESCONOCIDO_LEN, . - err_desconocido

det_argumentos:
        .ascii "uso_modulo_2_regresion_archivo_inicio_fin_columna"
...
det_denominador:
        .ascii "denominador_cero"
        .equ DET_DENOMINADOR_LEN, . - det_denominador
det_desconocido:
...
_start:
        ...
        //lee el rango de datos con utils
        ...
        //valida que haya datos suficientes
        cmp x1, #2
        blt error_sin_datos
        ldr x2, =conteo_guardado
        str x1, [x2]

        //calcula sumatorias con x de cero a n menos uno
        ldr x19, =valores
        mov x20, x1
        mov x21, #0
        mov x22, #0
...
sumar_valores:
        cmp x21, x20
        bge calcular_pendiente
        ldr x26, [x19, x21, lsl #3]
        add x22, x22, x21
        add x23, x23, x26
        mul x27, x21, x26
        add x24, x24, x27
...
calcular_pendiente:
        //guarda las sumatorias para depuracion interna
        ldr x0, =suma_x_guardada
        str x22, [x0]
        ldr x0, =suma_y_guardada
        str x23, [x0]
        ldr x0, =suma_xy_guardada
        str x24, [x0]
        ldr x0, =suma_xx_guardada
        str x25, [x0]

        //calcula el numerador y denominador
        mul x5, x20, x24
        mul x6, x22, x23
        sub x7, x5, x6
        ...
        mul x6, x22, x22
        sub x8, x5, x6
        cbz x8, error_denominador

        //escala la pendiente por cien
        mov x9, #100
        mul x7, x7, x9
        sdiv x10, x7, x8
        ldr x0, =pendiente_guardada
        str x10, [x0]

        //selecciona la tendencia
        cmp x10, #0
        bgt tendencia_subida
        blt tendencia_bajada
        ldr x0, =trend_stable
        mov x1, #TREND_STABLE_LEN
        b guardar_tendencia

tendencia_subida:
        ldr x0, =trend_ascending
        mov x1, #TREND_ASCENDING_LEN
        b guardar_tendencia

tendencia_bajada:
        ldr x0, =trend_descending
        mov x1, #TREND_DESCENDING_LEN

guardar_tendencia:
        ldr x2, =tendencia_texto
        str x0, [x2]
        ldr x2, =tendencia_largo
        str x1, [x2]
...
error_denominador:
        mov x0, #COD_DENOMINADOR_INVALIDO
...
imprimir_ok:
        ...
        //imprime el nombre del calculo
        ldr x0, =msg_calc
        mov x1, #MSG_CALC_LEN
        bl escribir_salida

        //imprime la columna solicitada
        ldr x0, =msg_column
        ...
        //imprime la pendiente escalada
        ldr x0, =msg_slope
        mov x1, #MSG_SLOPE_LEN
        ldr x2, =pendiente_guardada
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        //imprime la tendencia
        ldr x0, =msg_trend
        mov x1, #MSG_TREND_LEN
        bl escribir_salida
        ldr x0, =tendencia_texto
        ldr x0, [x0]
        ldr x1, =tendencia_largo
        ldr x1, [x1]
        bl escribir_salida
        bl imprimir_salto

        //imprime estado correcto
...
seleccionar_texto_error:
        ...
        mov x9, #COD_DENOMINADOR_INVALIDO
        cmp x0, x9
        beq texto_error_denominador
...
texto_error_limite:
        ...
        ret
texto_error_denominador:
        ldr x0, =err_denominador
        mov x1, #ERR_DENOMINADOR_LEN
        ret
...
texto_detalle_limite:
        ...
        ret
texto_detalle_denominador:
        ldr x0, =det_denominador
        mov x1, #DET_DENOMINADOR_LEN
        ret
