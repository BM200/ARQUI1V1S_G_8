...
msg_window_size:
        .ascii "WINDOW_SIZE="
        .equ MSG_WINDOW_SIZE_LEN, . - msg_window_size
msg_max_slope:
        .ascii "MAX_LOCAL_SLOPE_X100="
        .equ MSG_MAX_SLOPE_LEN, . - msg_max_slope
msg_error:
        .ascii "ERROR="
...
det_argumentos:
        .ascii "uso_modulo_5_derivada_local_archivo_inicio_fin_columna"
...
det_sin_datos:
        .ascii "se_necesitan_al_menos_cinco_datos"
...
_start:
        ...
        // esto calcula las pendientes locales
        ldr x19, =valores
        sub x20, x1, #4
        mov x21, #0
...
calcular_ventana:
        ...
        // esto carga una ventana de cinco puntos
        lsl x9, x21, #3
        add x10, x19, x9
        ldr x11, [x10]
        ldr x12, [x10, #8]
        ...
        // esto suma los valores de la ventana
        add x16, x11, x12
        add x16, x16, x13
        add x16, x16, x14
        add x16, x16, x15

        // esto calcula sumxy con x de cero a cuatro
        mov x17, #2
        mul x17, x13, x17
        add x17, x17, x12
        mov x18, #3
        madd x17, x14, x18, x17
        mov x18, #4
        madd x17, x15, x18, x17

        // esto calcula la pendiente local escalada por cien
        mov x18, #5
        mul x17, x17, x18
        mov x18, #10
        msub x17, x16, x18, x17
        mov x18, #100
        mul x17, x17, x18
        mov x18, #50
        sdiv x17, x17, x18
        ...
        // esto compara usando el valor absoluto
        mov x0, x26
        bl valor_absoluto
        mov x27, x0

        cbz x21, guardar_primera_pendiente
        ldr x9, =pendiente_abs_guardada
        ldr x10, [x9]
        cmp x27, x10
        ble siguiente_ventana
        str x27, [x9]
        ldr x9, =pendiente_guardada
        str x26, [x9]
        b siguiente_ventana

guardar_primera_pendiente:
        ldr x9, =pendiente_abs_guardada
        str x27, [x9]
        ldr x9, =pendiente_guardada
        str x26, [x9]
...
imprimir_ok:
        ...
        // esto imprime el tamano de ventana
        ldr x0, =msg_window_size
        mov x1, #MSG_WINDOW_SIZE_LEN
        mov x2, #TAM_VENTANA
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime la mayor pendiente local
        ldr x0, =msg_max_slope
        mov x1, #MSG_MAX_SLOPE_LEN
