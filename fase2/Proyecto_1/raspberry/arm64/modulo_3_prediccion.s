...
msg_intercept:
        ...
        .equ MSG_INTERCEPT_LEN, . - msg_intercept
msg_predicted:
        .ascii "PREDICTED_"
        .equ MSG_PREDICTED_LEN, . - msg_predicted
msg_equals:
        .ascii "="
        .equ MSG_EQUALS_LEN, . - msg_equals
msg_error:
        .ascii "ERROR="
...
err_k_invalido:
        .ascii "invalid_k"
        .equ ERR_K_INVALIDO_LEN, . - err_k_invalido
err_desconocido:
        .ascii "unknown_error"
        .equ ERR_DESCONOCIDO_LEN, . - err_desconocido

det_argumentos:
        .ascii "uso_modulo_3_prediccion_archivo_inicio_fin_columna_k_opcional"
...
det_k:
        .ascii "k_debe_ser_entero_positivo"
        .equ DET_K_LEN, . - det_k
det_desconocido:
...
leer_argumentos:
        ...
        // esto calcula sumatorias con x de cero a n menos uno
        ldr x19, =valores
        mov x20, x1
        mov x21, #0
        mov x22, #0
...
calcular_modelo:
        // esto guarda las sumatorias para depuracion interna
        ...
        // esto calcula numerador y denominador
        mul x5, x20, x24
        mul x6, x22, x23
        sub x7, x5, x6
        mul x5, x20, x25
        ...
        // esto calcula la pendiente escalada por cien
        mov x9, #100
        mul x7, x7, x9
        sdiv x10, x7, x8
        ...
        // esto calcula el intercepto escalado por cien
        mov x11, #100
        mul x12, x23, x11
        mul x13, x10, x22
        sub x12, x12, x13
        sdiv x12, x12, x20
        ldr x0, =intercepto_guardado
        str x12, [x0]

        // esto calcula la prediccion futura
        ldr x13, =k_guardado
        ldr x13, [x13]
        sub x14, x20, #1
        add x14, x14, x13
...
imprimir_ok:
        ...
        // esto imprime el intercepto
        ldr x0, =msg_intercept
        mov x1, #MSG_INTERCEPT_LEN
        ldr x2, =intercepto_guardado
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime la prediccion con k en el nombre
        ldr x0, =msg_predicted
        mov x1, #MSG_PREDICTED_LEN
        bl escribir_salida
        ldr x0, =k_guardado
        ldr x0, [x0]
...
texto_error_denominador:
        ...
        ret
texto_error_k:
        ldr x0, =err_k_invalido
        mov x1, #ERR_K_INVALIDO_LEN
        ret
