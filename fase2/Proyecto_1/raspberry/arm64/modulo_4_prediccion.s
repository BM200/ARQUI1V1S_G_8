...
msg_initial:
        .ascii "INITIAL_VALUE="
        .equ MSG_INITIAL_LEN, . - msg_initial
msg_final:
        .ascii "FINAL_VALUE="
        .equ MSG_FINAL_LEN, . - msg_final
msg_diff:
        .ascii "TOTAL_DIFF="
        .equ MSG_DIFF_LEN, . - msg_diff
msg_avg_x100:
        .ascii "AVG_CHANGE_X100="
        .equ MSG_AVG_X100_LEN, . - msg_avg_x100
msg_next_x100:
        .ascii "NEXT_VALUE_X100="
        .equ MSG_NEXT_X100_LEN, . - msg_next_x100
msg_error:
        .ascii "ERROR="
...
det_argumentos:
        .ascii "uso_modulo_4_prediccion_archivo_inicio_fin_columna"
...
diferencia_guardada:
        .skip 8
promedio_x100_guardado:
        .skip 8
siguiente_x100_guardado:
        .skip 8
...
_start:
        ...
        // esto toma el valor inicial y final
        ldr x19, =valores
        ldr x20, [x19]
        sub x21, x1, #1
        ldr x22, [x19, x21, lsl #3]
        ldr x0, =inicial_guardado
        str x20, [x0]
        ldr x0, =final_guardado
        str x22, [x0]

        // esto calcula la diferencia total
        sub x23, x22, x20
        ldr x0, =diferencia_guardada
        str x23, [x0]

        // esto calcula el cambio promedio escalado por cien
        mov x24, #100
        mul x25, x23, x24
        sdiv x26, x25, x21
        ldr x0, =promedio_x100_guardado
        str x26, [x0]

        // esto calcula la prediccion siguiente escalada por cien
        mul x27, x22, x24
        add x27, x27, x26
        ldr x0, =siguiente_x100_guardado
        str x27, [x0]
...
imprimir_ok:
        ...
        // esto imprime el valor inicial
        ldr x0, =msg_initial
        mov x1, #MSG_INITIAL_LEN
        ldr x2, =inicial_guardado
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto
        ...
        ldr x0, =msg_final
        mov x1, #MSG_FINAL_LEN
        ldr x2, =final_guardado
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime la diferencia total
        ldr x0, =msg_diff
        mov x1, #MSG_DIFF_LEN
        ldr x2, =diferencia_guardada
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime el cambio promedio escalado
        ldr x0, =msg_avg_x100
        mov x1, #MSG_AVG_X100_LEN
        ldr x2, =promedio_x100_guardado
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto

        // esto imprime la prediccion escalada
        ldr x0, =msg_next_x100
        mov x1, #MSG_NEXT_X100_LEN
        ldr x2, =siguiente_x100_guardado
        ldr x2, [x2]
        bl escribir_campo_numero
        bl imprimir_salto
